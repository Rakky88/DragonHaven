import { boundedJson, commandUnavailableResponse, handleCommand, object, RpcFailure } from "./core.ts";
import { evaluate, project, prepareImport, initializeGame, ruleset } from "./bundle.generated.ts";
import { runBeforeWorkerDeadline } from "./deadline.ts";
...
async function fetchJson(path: string, init: RequestInit, maximum: number, signal: AbortSignal) {
  const result = await fetch(`${base}${path}`, { ...init, signal });
  return { status: result.status, body: await boundedJson(result.body, maximum) };
}

Deno.serve((request) => runBeforeWorkerDeadline((signal) => handleCommand(request, {
  ruleset, evaluate, project, prepareImport, initializeGame,
  authenticate: async (authorization) => {
    ...
    const result = await fetchJson("/auth/v1/user", { headers: { authorization, apikey: serviceKey } }, 65536, signal);
