-- Remove seasonal score/action ceilings while preserving token, ownership,
-- one-use settlement, physical input rates and elapsed-clock validation.
-- Birthday joins Sunwake and New Year as an endless, renewable attempt.
...
begin
  ...
  if p_score is null or p_correct_actions is null or p_total_actions is null
     ...
     or (attempt.trial_key not in ('sunwakeSurf','midnightChime','wishcakeTower') and
       p_duration_ms > 180000)
     ...
     -- Rhythm spawns at most two notes per deterministic 10 ms step.
     -- Preserve old timed clients, but long endless runs finish at three errors.
     or (attempt.trial_key = 'midnightChime' and (
       p_total_actions > (p_duration_ms / 10) * 2 + 3
       or p_total_actions - p_correct_actions > 3
       or (p_duration_ms > 81000 and p_total_actions - p_correct_actions <> 3)))
     -- The cake is endless and ends on its first miss. Timed old clients
     -- remain valid during rollout; old three-miss clients can still settle.
     or (attempt.trial_key = 'wishcakeTower' and (
       p_total_actions > p_duration_ms / 420 + 1
       or p_total_actions - p_correct_actions > 3
       or (p_duration_ms > 81000 and p_total_actions - p_correct_actions not in (1,3))))
     -- Timed games have no score/action total ceiling. Keep a generous
     -- physical input rate bound, alongside points/action and wall-clock checks.
     or (attempt.trial_key not in ('sunwakeSurf','midnightChime','wishcakeTower')
       and p_total_actions > p_duration_ms / 10 * 2 + 3)
...
begin
  ...
  update public.seasonal_trial_attempts
    ...
    where id = p_attempt_id and user_id = auth.uid()
      and trial_key in ('sunwakeSurf','midnightChime','wishcakeTower') and completed_at is null
