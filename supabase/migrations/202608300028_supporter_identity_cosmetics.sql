-- DragonHaven: allow the verified Founding Supporter identity cosmetics in
-- online profiles. They remain outside all random chest pools in the client.
...
alter table public.profiles
  drop constraint if exists profiles_title_id_check;
alter table public.profiles
  drop constraint if exists profiles_portrait_key_check;
alter table public.profiles
  drop constraint if exists profiles_portrait_id_check;
alter table public.profiles
...
alter table public.profiles add constraint profiles_title_id_check check (
  ...
  or title = 'title_supporter_founder'
);
alter table public.profiles add constraint profiles_portrait_id_check check (
  portrait_key ~ '^portrait_(00[1-9]|0[1-9][0-9]|100)$'
  or portrait_key = 'portrait_supporter_founder'
);
alter table public.profiles add constraint profiles_frame_key_check check (
  frame_key is null or frame_key = 'frame_supporter_founder'
);

-- Keep the three-argument overload for already released clients. It changes
-- only the fields those clients know and therefore preserves any chosen frame.
...
-- The RPC row types need to be recreated to expose frame_key. Dropping the
-- snapshot first avoids a dependency conflict and it is rebuilt below in the
...
returns table (
  ...
  portrait_key text, frame_key text, discovered_dragon_count bigint,
