-- Endless New Year: preserve token, lease, score-rate and one-claim validation.
-- No account authority flags, balances or existing high scores are changed.
...
begin
  ...
  if p_score is null or p_correct_actions is null or p_total_actions is null
     ...
     or (attempt.trial_key not in ('sunwakeSurf','midnightChime') and
...
begin
  ...
  update public.seasonal_trial_attempts
    ...
    where id = p_attempt_id and user_id = auth.uid()
      and trial_key in ('sunwakeSurf','midnightChime') and completed_at is null
