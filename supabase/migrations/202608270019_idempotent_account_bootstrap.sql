-- A fresh client can observe an auth event while its explicit sign-in refresh
-- is still completing. Treat the one-time legacy import as an idempotent
-- bootstrap command so a repeated request can never strand the social UI.
