-- Order display observations across ruleset upgrades even when no game action
-- changed the player's canonical revision. Old in-flight reads cannot rewind
-- the projection after an app/server update.
alter table private.game_engine_runtime add column ruleset_revision bigint not null default 1
  check (ruleset_revision > 0 and ruleset_revision <= 9007199254740991);

create function private.advance_game_ruleset_revision()
returns trigger language plpgsql set search_path = '' as $$
begin
  new.ruleset_revision := old.ruleset_revision +
    case when new.ruleset_sha256 is distinct from old.ruleset_sha256 then 1 else 0 end;
  new.updated_at := clock_timestamp();
  return new;
end $$;
revoke all on function private.advance_game_ruleset_revision() from public, anon, authenticated;
create trigger game_engine_runtime_revision before update on private.game_engine_runtime
  for each row execute function private.advance_game_ruleset_revision();
