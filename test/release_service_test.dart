...
void main() {
  test('About and update checks share the same release version', () {
    expect(AppInfo.version, '0.04.08');
    expect(AppInfo.displayVersion, 'v0.04.08');
  ...
  test('release comparison understands the displayed version format', () {
    ...
    expect(release('v0.04.08').isNewerThanInstalled, isFalse);
    expect(release('v0.04.09').isNewerThanInstalled, isTrue);
  ...
  test('the copy button uses one permanent latest APK link', () {
    ...
    expect(ReleaseConfig.installedVersion, '0.04.08');
    expect(
