...
void main() {
  test('About and update checks share the same release version', () {
    expect(AppInfo.version, '0.06.08');
    expect(AppInfo.displayVersion, 'v0.06.08');
  ...
  test('the visible version cannot lag behind pubspec', () {
    ...
    expect(AppInfo.buildNumber, int.parse(match.group(4)!));
  });
  ...
  test('release comparison understands the displayed version format', () {
    ...
    expect(release('v0.05.06').isNewerThanInstalled, isFalse);
    expect(release('v0.05.07').isNewerThanInstalled, isFalse);
    expect(release('v0.05.09').isNewerThanInstalled, isFalse);
    expect(release('v0.05.10').isNewerThanInstalled, isFalse);
    expect(release('v0.05.11').isNewerThanInstalled, isFalse);
    expect(release('v0.05.12').isNewerThanInstalled, isFalse);
    expect(release('v0.05.13').isNewerThanInstalled, isFalse);
    ...
    expect(release('v0.05.20').isNewerThanInstalled, isFalse);
    expect(release('v0.05.21').isNewerThanInstalled, isFalse);
    expect(release('v0.05.23').isNewerThanInstalled, isFalse);
    ...
    expect(release('v0.06.03').isNewerThanInstalled, isFalse);
    expect(release('v0.06.05').isNewerThanInstalled, isFalse);
    expect(release('v0.06.07').isNewerThanInstalled, isFalse);
    expect(release('v0.06.08').isNewerThanInstalled, isFalse);
    expect(release('v0.06.09').isNewerThanInstalled, isTrue);
  ...
  test('the copy button uses one permanent latest APK link', () {
    ...
    expect(ReleaseConfig.installedVersion, '0.06.08');
    expect(
