...
applicatiehealthcheck lokaal voorbereid als migratie 32, inclusief begrensde
productieworkflow, klok-/contractvalidatie en 348/348 groene tests. Productie
blijft bewust op 31/31 totdat staging en een nieuwe expliciete
productiemigratietoestemming volgen.**
...
| Fase 1 — monitoring en incidenten | circa 76% | Privacyarme diagnostiek, correlation IDs, redactiontests, dashboardspecificatie en incidentrunbook bestaan; de productie-Auth-check draait elk uur. De read-only applicatiecheck, strikte contract-/klokvalidatie en migratie 32 zijn lokaal groen op 348/348 tests | Migratie 32 eerst op staging bewijzen en na toestemming naar productie brengen; daarna de echte applicatiecheck en testalert bewijzen. Firebase Crashlytics/Performance koppelen zodra de Android-projectconfig bestaat | Gratis Firebase Spark-project maken, `nl.dragonhaven.app` registreren, Analytics uit laten en `google-services.json` veilig in de werkmap zetten; testalert ontvangen en privacy/Data Safety beoordelen |
...
- [ ] Activeer de veilige, read-only applicatiecheck. Migratie 32, de
  ...
  productiemigratie zijn lokaal klaar; staging- en productiebewijs wachten nog.
...
| 31-08-2026 | M1 read-only applicatiehealth lokaal voorbereid | Codex | migratie `202608310032_public_application_health.sql`, `public_server_health_check.ps1`, `release_server_preflight.ps1`, `test_public_application_health.ps1`, `production-migrate-32.yml` en `dragonhaven_spec_test.dart` | Een publieke `security invoker`-RPC zonder tabelreads retourneert uitsluitend vaste servicestatus, contractversie en servertijd. De monitor valideert HTTP-status, exact contract en maximaal vijf minuten klokafwijking; productiepreflight eist de check na migratie 32. Positieve/negatieve parsertests, PowerShell-parser, live Auth 200/200, analyzer en 348/348 Flutter-tests zijn groen. Productie blijft 31/31; staging, activering en testalert wachten nog. |
