/// German, Spanish, French, Italian, Portuguese and Japanese.
const birthdayPhraseTranslations = <String, List<String>>{
  "Bake a Wish to the Stars": [
    "Backe einen Wunsch bis zu den Sternen",
    "Hornea un deseo hasta las estrellas",
    "Un gâteau de vœux jusqu’aux étoiles",
    "Un dolce desiderio fino alle stelle",
    "Um bolo de desejos até às estrelas",
    "星まで届く願いのケーキ"
  ],
  "Tap to drop the moving cake layer onto the one below. Overhanging cake falls away, so aim for a perfect fit. Three perfect layers restore a little width. The tower gets faster; three missed layers end the Trial.":
      [
    "Tippe, um die bewegliche Kuchenschicht auf die darunterliegende fallen zu lassen. Überstehende Stücke fallen ab. Drei perfekte Schichten geben etwas Breite zurück. Das Tempo steigt; drei verfehlte Schichten beenden die Prüfung.",
    "Toca para dejar caer la capa sobre la de abajo. Lo que sobresale se pierde. Tres capas perfectas recuperan un poco de anchura. La torre acelera; tres fallos terminan la prueba.",
    "Touche pour poser la couche mobile sur celle du dessous. Les morceaux qui dépassent tombent. Trois couches parfaites rendent un peu de largeur. Le rythme accélère ; trois couches ratées terminent l’épreuve.",
    "Tocca per far cadere lo strato su quello sotto. Le parti sporgenti cadono. Tre strati perfetti restituiscono un po’ di larghezza. La torre accelera; tre errori terminano la prova.",
    "Toca para largar a camada sobre a de baixo. As partes que sobram caem. Três camadas perfeitas recuperam alguma largura. O ritmo aumenta; três falhas terminam a prova.",
    "タップで動くケーキの層を下の層に重ねよう。はみ出した部分は落ちます。3回連続でぴったり重ねると幅が少し回復。次第に速くなり、3回外すと終了。"
  ],
  "Stack a birthday wish": [
    "Stapele einen Geburtstagswunsch",
    "Apila un deseo de cumpleaños",
    "Empile un vœu d’anniversaire",
    "Impila un desiderio di compleanno",
    "Empilha um desejo de aniversário",
    "誕生日の願いを積み重ねよう"
  ],
  "Only the overlapping slice stays": [
    "Nur der überlappende Teil bleibt",
    "Solo queda la parte superpuesta",
    "Seule la partie superposée reste",
    "Resta solo la parte sovrapposta",
    "Só fica a parte sobreposta",
    "重なった部分だけが残ります"
  ],
  "Perfect fit!": [
    "Perfekt gestapelt!",
    "¡Encaja a la perfección!",
    "Parfaitement aligné !",
    "Perfettamente allineato!",
    "Encaixe perfeito!",
    "ぴったり！"
  ],
  "Keep your next layer centered": [
    "Zentriere die nächste Schicht",
    "Centra la siguiente capa",
    "Centre bien la couche suivante",
    "Centra il prossimo strato",
    "Centra a próxima camada",
    "次の層は中央に重ねよう"
  ],
  "Missed! Try the fresh cake base": [
    "Verfehlt! Versuche es auf dem neuen Boden",
    "¡Fallaste! Prueba con la nueva base",
    "Raté ! Essaie sur la nouvelle base",
    "Mancato! Prova sulla nuova base",
    "Falhou! Tenta na nova base",
    "外れた！新しい土台で再挑戦"
  ],
  "Drop layer": [
    "Schicht fallen lassen",
    "Soltar capa",
    "Poser la couche",
    "Lascia lo strato",
    "Largar camada",
    "層を落とす"
  ],
  "Another layer of wishes!": [
    "Noch eine Schicht voller Wünsche!",
    "¡Otra capa de deseos!",
    "Une couche de vœux de plus !",
    "Un altro strato di desideri!",
    "Mais uma camada de desejos!",
    "願いをもう一段！"
  ],
  "Layer missed - 30 points lost": [
    "Schicht verfehlt - 30 Punkte verloren",
    "Capa fallida - pierdes 30 puntos",
    "Couche ratée - 30 points perdus",
    "Strato mancato - persi 30 punti",
    "Camada falhada - menos 30 pontos",
    "層を外した - 30ポイント減点"
  ]
};
