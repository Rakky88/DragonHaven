      ...
      Future<void> settle({bool permitLost = false}) async {
        ...
        if (!permitLost &&
          ...
          await tester.runAsync(() async {
            ...
            if (pending != null) {
              stdout.writeln('PROBE: group_recover_same_pending_request');
              try {
                final receipt = await games[active].synchronize();
                stdout.writeln(
                    'PROBE: group_pending_${receipt?.replayed == true ? 'replayed' : 'resolved'}');
              } on Object {
                stdout.writeln('PROBE: group_pending_still_unavailable');
              }
            }
          });
        }
