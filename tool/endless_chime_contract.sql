...
begin
  if has_function_privilege('anon','public.renew_seasonal_trial_attempt(uuid,text)','execute') then
    raise exception 'chime_contract_anonymous_renewal'; end if;
  ...
  insert into public.seasonal_trial_attempts(user_id,event_id,trial_key,occurrence_key,
    ...
    values(keeper,'new_year_first_dawn','midnightChime','new_year_first_dawn:2027',5,false,
  ...
  begin
    ...
    raise exception 'chime_contract_foreign_renewal';
  ...
  begin
    ...
    raise exception 'chime_contract_wrong_token';
  exception when others then if sqlerrm<>'seasonal_attempt_unavailable' then raise; end if; end;
  extended := public.renew_seasonal_trial_attempt(a.id,a.completion_token::text);
  if extended <> now()+interval '6 hours' then raise exception 'chime_contract_lease'; end if;
  begin
    perform public.complete_seasonal_trial_attempt(a.id,a.completion_token::text,50000,250,252,200000);
    raise exception 'chime_contract_missing_third_mistake';
  exception when others then if sqlerrm<>'seasonal_score_rejected' then raise; end if; end;
  begin
    perform public.complete_seasonal_trial_attempt(a.id,a.completion_token::text,50000,5000,5003,10000);
    raise exception 'chime_contract_impossible_gate_rate';
  exception when others then if sqlerrm<>'seasonal_score_rejected' then raise; end if; end;
  begin
    perform public.complete_seasonal_trial_attempt(a.id,a.completion_token::text,60000,250,253,200000);
    raise exception 'chime_contract_impossible_points';
  exception when others then if sqlerrm<>'seasonal_score_rejected' then raise; end if; end;
  ...
  if not r.accepted or r.simulated or r.best_score<>50000 then
    raise exception 'chime_contract_long_run'; end if;
  begin
    perform public.renew_seasonal_trial_attempt(a.id,a.completion_token::text);
    raise exception 'chime_contract_finished_renewal';
  exception when others then if sqlerrm<>'seasonal_attempt_unavailable' then raise; end if; end;
  begin
    perform public.complete_seasonal_trial_attempt(a.id,a.completion_token::text,50000,250,253,28800000);
    raise exception 'chime_contract_replay';
  ...
  begin
    perform public.renew_seasonal_trial_attempt(a.id,a.completion_token::text);
    raise exception 'chime_contract_expired_renewal';
  exception when others then if sqlerrm<>'seasonal_attempt_unavailable' then raise; end if; end;
  ...
  begin
    perform public.complete_seasonal_trial_attempt(a.id,a.completion_token::text,50000,250,253,28800000);
    raise exception 'chime_contract_other_game';
  exception when others then if sqlerrm<>'seasonal_score_rejected' then raise; end if; end;
end;
$$;
rollback;
select true as chime_contract_passed;
