...
begin
  if has_function_privilege('anon','public.acknowledge_my_privacy_notice(text,boolean)','execute')
     or has_function_privilege('anon','public.get_my_privacy_acknowledgement_for_version(text)','execute')
  ...
  if public.get_my_privacy_acknowledgement_for_version('2026-09-23')
      then raise exception 'privacy_contract_current_default'; end if;
  begin
    perform public.get_my_privacy_acknowledgement_for_version('2026-09-20');
    raise exception 'privacy_contract_exact_version';
  ...
  if not public.get_my_privacy_acknowledgement()
      then raise exception 'privacy_contract_legacy_ack'; end if;
  if public.get_my_privacy_acknowledgement_for_version('2026-09-23')
      then raise exception 'privacy_contract_legacy_unlocks_ads'; end if;
  perform public.acknowledge_my_privacy_notice('2026-09-23',true);
  if not public.get_my_privacy_acknowledgement()
      or not public.get_my_privacy_acknowledgement_for_version('2026-09-23')
      then raise exception 'privacy_contract_current_ack'; end if;
  select acknowledged_at into accepted_at from private.account_privacy_acknowledgements where user_id=a;
  perform public.acknowledge_my_privacy_notice('2026-09-20',true);
  if not exists(select 1 from private.account_privacy_acknowledgements
      where user_id=a and notice_version='2026-09-23' and acknowledged_at=accepted_at)
      then raise exception 'privacy_contract_downgrade'; end if;
  perform public.acknowledge_my_privacy_notice('2026-09-23',true);
