...
try {
  if ($RehearseMigrations) {
    ...
    if ($remote[-1] -notin @('202609090065','202609100066','202609100067','202609100068','202609100069') -or
    ...
    $pending = @($files | Where-Object { [long]$_.BaseName.Split('_')[0] -gt [long]$remote[-1] -and
        [long]$_.BaseName.Split('_')[0] -le 202609100069 } | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw -Encoding utf8 })
