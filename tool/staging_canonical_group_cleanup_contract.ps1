...
$query = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'canonical_group_cleanup_contract.sql') -Raw -Encoding utf8
$phase = 'history'
try {
  if ($RehearseMigrations) {
    ...
    if ($remote[-1] -notin @('202609090065','202609100066','202609100067','202609100068','202609100069','202609100070','202609100071','202609100072','202609100073','202609100074','202609100075','202609100076','202609100077','202609100078','202609100079') -or
        @(Compare-Object $remote $expected).Count -ne 0) { throw 'group_cleanup_contract_schema'; }
    $pending = @($files | Where-Object { [long]$_.BaseName.Split('_')[0] -gt [long]$remote[-1] -and
        [long]$_.BaseName.Split('_')[0] -le 202609100077 } | ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw -Encoding utf8 })
  ...
  if ($detail -match '\b(group_cleanup_contract_[a-z_]+)\b') { Write-Output "Failed assertion: $($Matches[1])" }
  ...
  throw 'Staging canonical group cleanup rehearsal failed; its transaction must roll back.'
}
if ($result.canonical_group_cleanup_contract_passed -ne $true) { throw 'canonical group cleanup contract did not confirm rollback.' }
Write-Output "PASS: canonical group cleanup; all catalog requirements attainable, exact shared lifecycle, promoted participant/owner Auth deletion, other inventories unchanged and standalone inventory deletion refused; migrations_rehearsed=$RehearseMigrations; synthetic_changes_rolled_back=true"
