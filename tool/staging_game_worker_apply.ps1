...
$baseline = @($local | Where-Object { [long]$_ -le 202609090062 })
if ($local[-1] -cne '202609090063' -or
    (@(Compare-Object $remote $baseline).Count -ne 0 -and @(Compare-Object $remote $local).Count -ne 0)) {
  throw 'Requires exact registered staging schema 62/63 and only birthday migration 63.'
}
./tool/staging_birthday_feature_contract.ps1 -ProjectRef $projectRef `
  -ManagementAccessToken $env:STAGING_SUPABASE_ACCESS_TOKEN `
  -RehearseMigrations:($remote[-1] -ceq '202609090062')
