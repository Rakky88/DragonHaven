...
$baseline = @($local | Where-Object { [long]$_ -le 202609090061 })
if ($local[-1] -cne '202609090062' -or
    (@(Compare-Object $remote $baseline).Count -ne 0 -and @(Compare-Object $remote $local).Count -ne 0)) {
  throw 'Requires exact registered staging schema 61/62 and only event migration 62.'
}
./tool/staging_release25_feature_contract.ps1 -ProjectRef $projectRef `
  -ManagementAccessToken $env:STAGING_SUPABASE_ACCESS_TOKEN `
  -RehearseMigrations:($remote[-1] -ceq '202609090061')
