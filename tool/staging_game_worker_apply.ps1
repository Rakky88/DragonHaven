...
if ($local[-1] -cne '202609100076' -or $local.Count -ne 76 -or
    $remote[-1] -notin @('202609090065','202609100066','202609100067','202609100068','202609100069','202609100070','202609100071','202609100072','202609100073','202609100074','202609100075','202609100076') -or
    @(Compare-Object $remote $expectedRemote).Count -ne 0) {
  throw 'Requires exact registered staging schema 65 through 76 and the reviewed canonical migrations.'
}
...
./tool/staging_wide_trial_score_contract.ps1 -ProjectRef $projectRef `
  -ManagementAccessToken $env:STAGING_SUPABASE_ACCESS_TOKEN `
  -RehearseMigrations:([long]$remote[-1] -lt 202609100075)
