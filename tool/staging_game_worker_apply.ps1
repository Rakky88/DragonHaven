...
$baseline = @($local | Where-Object { [long]$_ -le 202609090060 })
if ($local[-1] -cne '202609090061' -or
    (@(Compare-Object $remote $baseline).Count -ne 0 -and @(Compare-Object $remote $local).Count -ne 0)) {
  throw 'Requires exact registered staging schema 60/61 and only event migration 61.'
}
./tool/staging_release24_feature_contract.ps1 -ProjectRef $projectRef `
  -ManagementAccessToken $env:STAGING_SUPABASE_ACCESS_TOKEN `
  -RehearseMigrations:($remote[-1] -ceq '202609090060')
