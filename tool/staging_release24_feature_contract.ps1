...
$query = Get-Content -LiteralPath 'tool/seasonal_three_strikes_contract.sql' -Raw
if ($RehearseMigrations) {
  $query = "begin;`n" + (Get-Content -LiteralPath 'supabase/migrations/202609090061_seasonal_three_strikes.sql' -Raw) + "`n" +
...
if ($result.three_strikes_contract_passed -ne $true) { throw 'Three-strike contract did not confirm rollback.' }
Write-Output "PASS: three_strikes_contract_passed; migrations_rehearsed=$RehearseMigrations; synthetic_changes_rolled_back=true"
