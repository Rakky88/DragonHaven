...
$query = Get-Content -LiteralPath 'tool/end_event_contract.sql' -Raw
if ($RehearseMigrations) {
  $query = "begin;`n" + (Get-Content -LiteralPath 'supabase/migrations/202609090060_end_active_event.sql' -Raw) + "`n" +
...
if ($result.end_event_contract_passed -ne $true) { throw 'Event contract did not confirm rollback.' }
Write-Output "PASS: end_event_contract_passed; migrations_rehearsed=$RehearseMigrations; synthetic_changes_rolled_back=true"
