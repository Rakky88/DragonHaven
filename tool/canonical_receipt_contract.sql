...
declare keeper uuid := gen_random_uuid(); outsider uuid := gen_random_uuid();
  v_request_id uuid := gen_random_uuid(); second_request uuid := gen_random_uuid(); third_request uuid := gen_random_uuid();
...
begin
  ...
  update private.game_engine_runtime set enabled=false, ruleset_sha256=repeat('cd',32) where singleton;
  replay := public.begin_canonical_game_command(keeper,v_request_id,'purchase_title_chest','{}',10068,ruleset);
  if replay->>'status' <> 'succeeded' or replay->'response' <> receipt
    or replay ? 'secret_seed' or replay ? 'state' or replay ? 'lease_token' then
    raise exception 'receipt_contract_paused_replay';
  end if;
  replay := public.begin_canonical_game_command(keeper,second_request,'purchase_title_chest','{}',10068,ruleset);
  if replay->>'status' <> 'failed' or replay->>'failure_code' <> 'invalid_command' then
    raise exception 'receipt_contract_paused_failure';
  end if;
  begin
    perform public.begin_canonical_game_command(keeper,v_request_id,'purchase_title_chest','{"extra":true}',10068,ruleset);
    raise exception 'receipt_contract_paused_conflict';
  exception when others then if sqlerrm <> 'game_idempotency_conflict' then raise; end if; end;
  begin
    perform public.begin_canonical_game_command(keeper,third_request,'purchase_title_chest','{}',10068,ruleset);
    raise exception 'receipt_contract_paused_new_lease';
  exception when others then if sqlerrm <> 'game_engine_disabled' then raise; end if; end;
  if (select count(*) from private.canonical_game_intents where owner_id=keeper) <> 2
    or (select revision from private.canonical_game_states where owner_id=keeper) <> 2 then
    raise exception 'receipt_contract_replay_changed_state';
  end if;
  update private.game_engine_runtime set enabled=true, ruleset_sha256=ruleset where singleton;
  started := public.begin_canonical_game_command(keeper,third_request,'purchase_title_chest','{}',10068,ruleset);
  old_token := (started->>'lease_token')::uuid;
  update private.canonical_game_intents set leased_until=now()-interval '1 second'
    where owner_id=keeper and request_id=third_request;
  update private.game_engine_runtime set enabled=false where singleton;
  begin
    perform public.begin_canonical_game_command(keeper,third_request,'purchase_title_chest','{}',10068,ruleset);
    raise exception 'receipt_contract_paused_lease_reacquired';
  exception when others then if sqlerrm <> 'game_engine_disabled' then raise; end if; end;
  if (select lease_token from private.canonical_game_intents where owner_id=keeper and request_id=third_request) <> old_token then
    raise exception 'receipt_contract_paused_lease_changed';
  end if;
  update private.game_engine_runtime set enabled=true, ruleset_sha256=repeat('cd',32) where singleton;
  begin
    perform public.begin_canonical_game_command(keeper,third_request,'purchase_title_chest','{}',10068,repeat('cd',32));
    raise exception 'receipt_contract_pending_rules_changed';
  exception when others then if sqlerrm <> 'game_pending_ruleset_changed' then raise; end if; end;
...
$$;
rollback;
select true as canonical_receipt_contract_passed;
