"""Bounded release-39 rehearsal. Every synthetic fixture is rolled back."""
import argparse
import json
import os
...
def main():
    ...
    if environment not in expected or project != expected[environment] or not token:
        raise RuntimeError('release39_configuration_invalid')
    ...
    def query(sql, readonly=False):
        ...
        try:
            ...
            assertion = re.search(r'\b(?:expertise|chime|event_points)_contract_[a-z_]+\b', body)
            state = re.search(r'ERROR:\s+([0-9A-Z]{5}):', body)
            if assertion:
                print('Failed assertion:', assertion[0])
            if state:
                print('SQL state:', state[1])
            raise RuntimeError('release39_contract_query_failed') from None

    files = sorted(Path('supabase/migrations').glob('*.sql'))
    local = [p.name.split('_')[0] for p in files]
    if len(local) != 86 or local[-3:] != ['202609130084', '202609200085', '202609200086']:
        raise RuntimeError('release39_local_history_invalid')
    versions = [r['version'] for r in query(
        'select version from supabase_migrations.schema_migrations order by version', True)]
    if len(versions) not in range(83, 87) or versions != local[:len(versions)]:
        raise RuntimeError('release39_remote_history_invalid')
    if args.phase == 'after' and versions != local:
        raise RuntimeError('release39_apply_incomplete')
    ...
    if len(authority) != 1 or any(authority[0].values()):
        raise RuntimeError('release39_requires_dormant_authority')
    folder = Path('release39-schema')
    ...
    if args.phase == 'before':
        ...
        raise RuntimeError('release39_authority_changed')
    ...
    for name, field in [
        ...
        if result != [{field: True}]:
            raise RuntimeError('release39_rollback_not_confirmed')
    ...
    if query('select version from supabase_migrations.schema_migrations order by version', True) != [
        ...
        raise RuntimeError('release39_rehearsal_changed_history')
...
if __name__ == '__main__':
    try:
        ...
        print(str(error) if isinstance(error, RuntimeError) else 'release39_contract_unavailable')
