...
begin
  if (select enabled or shadow_social_enabled or shadow_projection_enabled or shadow_lifecycle_enabled
      from private.game_engine_runtime where singleton) then raise exception 'group_cleanup_contract_requires_dormant'; end if;
  if has_function_privilege('service_role','public.begin_canonical_game_command(uuid,uuid,text,jsonb,integer,text)','execute')
    ...
    raise exception 'group_cleanup_contract_permissions'; end if;
  ...
  if exists(select 1 from unnest(keepers) k where not private.canonical_group_keeper_ready(k)) then
    raise exception 'group_cleanup_contract_prepared_read_models'; end if;
  ...
  if target_lobby is null or exists(select 1 from public.group_adventure_lobbies where id=target_lobby) then
    raise exception 'group_cleanup_contract_prepare_mutated_source'; end if;
  ...
  begin
    ...
    raise exception 'group_cleanup_contract_wrong_receipt';
  ...
  if (select state from private.canonical_game_states where owner_id=keepers[1])<>current_state
    ...
    raise exception 'group_cleanup_contract_partial_create'; end if;
  ...
  if retried->>'status'<>'succeeded' or retried->'response'<>receipt
    ...
    raise exception 'group_cleanup_contract_create_replay'; end if;
  ...
  if not running.canonical_owned or running.status<>'waiting'
    ...
    raise exception 'group_cleanup_contract_catalog_terms'; end if;
  ...
  if result_value->'result'->>'accepted'<>'true' then raise exception 'group_cleanup_contract_join'; end if;
  ...
  if result_value->'result'->>'accepted'<>'true'
    ...
    raise exception 'group_cleanup_contract_kick_reservation'; end if;
  ...
  result_value:=pg_temp.group_step(keepers[2],'join_group_adventure',jsonb_build_object('lobbyId',target_lobby,'dragonId','crew-2'));
  if result_value->'result'->>'accepted'<>'true' then raise exception 'group_cleanup_contract_rejoin'; end if;
  ...
  if result_value->'result'->>'accepted'<>'true'
    ...
    raise exception 'group_cleanup_contract_leave'; end if;
  ...
  begin
    ...
    raise exception 'group_cleanup_contract_changed_friendship_committed';
  exception when others then if sqlerrm<>'game_social_state_changed' then raise; end if; end;
  ...
  if result_value->'result'->>'accepted'<>'true' or running.status<>'running'
    ...
    raise exception 'group_cleanup_contract_shared_start'; end if;
  result_value:=pg_temp.group_step(keepers[1],'leave_group_adventure',jsonb_build_object('lobbyId',target_lobby));
  if result_value->>'failure_code'<>'game_action_unavailable' then raise exception 'group_cleanup_contract_running_leave'; end if;
  ...
  if not exists(select 1 from public.group_adventure_lobbies l where l.id=target_lobby and status='completed') then
    raise exception 'group_cleanup_contract_legacy_clock_maintenance'; end if;
  begin
    delete from public.group_adventure_participants p where p.lobby_id=target_lobby and p.user_id=keepers[2];
    raise exception 'group_cleanup_contract_legacy_member_write';
  ...
  if exists(select 1 from private.canonical_game_states where owner_id=any(keepers)
    ...
    raise exception 'group_cleanup_contract_unearned_rewards'; end if;
  perform set_config('request.jwt.claim.role','service_role',true);
  -- Every catalog requirement fits that adventure's number of level-nine dragons.
  for index_value in 0..199 loop
    insert into public.group_adventure_lobbies(slot,adventure_id,owner_id,required_players,focus,
        base_duration_minutes,xp,stat_points,combined_level_required,combined_stat_required)
      values(1000000+index_value,'group_'||(index_value+1),keepers[1],2+index_value%3,'might',4320,885,91,
        case when index_value%4=0 then 8+index_value%20 else 0 end,0);
  end loop;
  if exists(select 1 from public.group_adventure_lobbies where owner_id=keepers[1]
      and combined_level_required>required_players*9) or
      not exists(select 1 from public.group_adventure_lobbies where owner_id=keepers[1] and slot=1000012
        and required_players=2 and combined_level_required=18) then
    raise exception 'group_cleanup_contract_attainable_requirements'; end if;
  update public.player_economy_authority set authority_mode='server',activated_at=now() where user_id=any(keepers);
  perform set_config('request.jwt.claim.role','authenticated',true);
  perform set_config('request.jwt.claim.sub',keepers[2]::text,true);
  begin
    delete from public.player_dragons where owner_id=keepers[2];
    raise exception 'group_cleanup_contract_standalone_inventory_delete';
  exception when others then if sqlerrm<>'economy_server_inventory_required' then raise; end if; end;
  perform public.delete_my_account();
  if exists(select 1 from auth.users where id=keepers[2]) or
      exists(select 1 from public.group_adventure_participants where user_id=keepers[2]) or
      not exists(select 1 from public.group_adventure_lobbies where id=target_lobby) then
    raise exception 'group_cleanup_contract_participant_auth_cascade'; end if;
  perform set_config('request.jwt.claim.sub',keepers[1]::text,true);
  perform public.delete_my_account();
  if exists(select 1 from auth.users where id=keepers[1]) or
      exists(select 1 from public.group_adventure_lobbies where owner_id=keepers[1]) or
      exists(select 1 from public.group_adventure_participants where lobby_id=target_lobby) or
      (select count(*) from private.canonical_game_states where owner_id=any(keepers))<>2 or
      exists(select 1 from private.canonical_game_states where owner_id in(keepers[3],keepers[4])
        and ((state->'pet'->>'coins')::integer<>1000 or (state->'pet'->>'xp')::integer<>3400)) then
    raise exception 'group_cleanup_contract_owner_auth_cascade'; end if;
end $$;
rollback;
select true as canonical_group_cleanup_contract_passed;
