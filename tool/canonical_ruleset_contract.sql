...
declare v_owner uuid := gen_random_uuid(); v_other uuid := gen_random_uuid();
  ...
  v_ruleset_revision bigint; v_rules text := repeat('ab',32); v_count bigint;
begin
  ...
  select ruleset_revision into v_ruleset_revision from private.game_engine_runtime where singleton;
  update private.game_engine_runtime set ruleset_revision=0 where singleton;
  if (select ruleset_revision from private.game_engine_runtime where singleton) <> v_ruleset_revision then
    raise exception 'ruleset_contract_manual_revision_change';
  end if;
  update private.game_engine_runtime set minimum_client_build=minimum_client_build where singleton;
  if (select ruleset_revision from private.game_engine_runtime where singleton) <> v_ruleset_revision then
    raise exception 'ruleset_contract_noop_incremented';
  end if;
  update private.game_engine_runtime set ruleset_sha256=
    case when ruleset_sha256=repeat('ab',32) then repeat('cd',32) else repeat('ab',32) end where singleton;
  if (select ruleset_revision from private.game_engine_runtime where singleton) <> v_ruleset_revision+1 then
    raise exception 'ruleset_contract_upgrade_not_incremented';
  end if;
  ...
  if (select ruleset_revision from private.game_engine_runtime where singleton) <> v_ruleset_revision+1 then
    raise exception 'ruleset_contract_pause_incremented';
  end if;
  ...
  if (v_snapshot->>'ruleset_revision')::bigint is distinct from
    (select ruleset_revision from private.game_engine_runtime where singleton) then
    raise exception 'ruleset_contract_read_generation';
  end if;
...
rollback;
select true as canonical_ruleset_contract_passed;
